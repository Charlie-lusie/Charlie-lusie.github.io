...
1. 为点云中的每个点求它的表面法向量。因为点云数据是离散点，所以只能计算出近似的表面法向量[^foot1]。这里选用$k$近邻点进行平面拟合[^foot2]。对于点$p_i$ ，假设$p_i$与最近邻的$k$个点产生的协方差矩阵为$C$，$C​$的计算公式如下：
[^foot1]: 点云法向量估计可参考: http://hhoppe.com/recon.pdf
[^foot2]: 最小二乘拟合平面可参考： http://www.ilikebigbits.com/2017_09_25_plane_from_points_2.html
$$
...
2. 设定一个邻域半径值$r$，对每个点选取半径范围内的所有点用于计算该点的点特征直方图。对于两个点$p_i$和$p_j$，各自的表面法向量为$n_i$和$n_j$，对这两个点建立一个如图1所示的局部空间坐标系$(u,v,w)​$，计算如下：
$$
u=n_j\\
